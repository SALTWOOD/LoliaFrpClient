...
public sealed partial class LineChart : UserControl
{
    public static readonly DependencyProperty TitleProperty =
        DependencyProperty.Register(nameof(Title), typeof(string), typeof(LineChart),
            new PropertyMetadata(string.Empty));

    public static readonly DependencyProperty DataProperty =
        DependencyProperty.Register(nameof(Data), typeof(List<DailyTrafficViewModel>), typeof(LineChart),
    ...
    {
        if (d is LineChart chart) chart.DrawChart();
    }
    ...
    {
        if (Data == null || Data.Count == 0 || ChartCanvas == null) return;
        ...
        // 计算最大值用于缩放
        var maxValue = Data.Max(d => Math.Max(d.InboundBytes, d.OutboundBytes));
    ...
    {
        var gridColor = new SolidColorBrush(Color.FromArgb(255, 200, 200, 200));
    ...
    private void DrawLine(List<long> values, long maxValue, double width, double height, Brush strokeBrush,
        string label)
    {
        ...
        for (var i = 0; i < values.Count; i++)
        {
            ...
            var y = height - height * values[i] / maxValue;
    ...
    private void DrawInteractivePoints(long maxValue, double width, double height)
    {
        ...
        for (var i = 0; i < dataCount; i++)
        {
            var x = width * i / Math.Max(1, dataCount - 1);
            var data = Data[i];
    ...
    private void OnPointPointerEntered(object sender, PointerRoutedEventArgs e)
    {
        if (sender is Rectangle rect && rect.Tag is int index && index >= 0 && index < Data.Count)
        {
            ...
            // 确保 Popup 有 XamlRoot
            if (_tooltipPopup!.XamlRoot == null) _tooltipPopup.XamlRoot = XamlRoot;

            // 计算 Tooltip 位置
            var popupX = position.X + 10;
            var popupY = position.Y - 60;

            // 确保 Tooltip 不超出画布边界
            _tooltipBorder!.Measure(new Size(double.PositiveInfinity, double.PositiveInfinity));
            var tooltipWidth = _tooltipBorder.DesiredSize.Width;
            var tooltipHeight = _tooltipBorder.DesiredSize.Height;

            if (popupX + tooltipWidth > ChartCanvas.ActualWidth) popupX = position.X - tooltipWidth - 10;
            if (popupY < 0) popupY = position.Y + 20;

            // 转换为屏幕坐标
            var transform = ChartCanvas.TransformToVisual(XamlRoot.Content);
            var screenPoint = transform.TransformPoint(new Point(popupX, popupY));
